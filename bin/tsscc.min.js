...
o+"'");a.push(r[o].args[l].def)}else a.push(h.parameter),s.offset=h.end+1}this.logMmlCompile&&Log.getLog().info(a),s.dataLength=s.data.length,r[o].callback&&r[o].callback(this,s,u,a);if(this.logMmlCompile){var p="> "+s.dataLength.toString(16)+": ";for(l=s.dataLength;l<s.data.length;l++)l!=s.dataLength&&(p+=", "),p+=s.data[l].toString(16);Log.getLog().info(p)}s.dataLength=s.data.length}}s.data.push(TsdPlayer.CMD_END),this.channelData[i]=s.data,Log.getLog().info("TSS: DATA "+(i+1)+"> "+s.data.length+" Byte(s) / "+s.count+" Tick(s)")}},TssCompiler.prototype._generateTsd=function(){var e=this.tags.title.byteLength();0!=e%2&&e++;var t=18+e+2+8*this.tags.channels+4,n=t;Log.getLog().info("TSS: HEADER SIZE> "+n);var r;for(r=0;r<this.tags.channels;r++)n+=this.channelData[r].length;var i=n;n+=2,Log.getLog().info("TSS: WAVE> "+this.validWaves);for(r=0;r<this.waves.length;r++){if(!this.waves[r])continue;Log.getLog().info("TSS:  "+r+"> "+this.waves[r].length),n+=2+this.waves[r].length}n+=2,Log.getLog().info("TSS: TABLE> "+this.validTables);for(r=0;r<this.tables.length;r++){if(!this.tables[r])continue;Log.getLog().info("TSS:  "+r+"> "+this.tables[r].length),n+=2+this.tables[r].length}var s=new Uint8Array(n);Log.getLog().info("TSS: TOTAL SIZE> "+n);var o=TString.createFromUint8Array(s),u=o.setASCII(0,"T'SoundSystem");o.setAt(u++,Math.floor(TssCompiler.VERSION)),o.setAt(u++,Math.floor(TssCompiler.VERSION*100)%100),u=o.setUint16(u,this.tags.title.byteLength()),u=o.setTString(u,this.tags.title),0==this.tags.title.byteLength()%2&&u--,u=o.setUint16(u,this.tags.channels);var a=t;for(r=0;r<this.tags.channels;r++){var f=this.channelData[r].length;u=o.setUint32(u,a),u=o.setUint32(u,f);for(var l=0;l<f;l++)o.setAt(a+l,this.channelData[r][l]);a+=f}u=o.setUint32(u,i),u=o.setUint16(i,this.validWaves);for(r=0;r<this.validWaves;r++){if(!this.waves[r])continue;o.setAt(u++,r);var c=this.waves[r].length;o.setAt(u++,c);for(var h=0;h<c;h++)o.setAt(u++,TssCompiler._toUint8(this.waves[r][h]))}u=o.setUint16(u,this.validTables);for(r=0;r<this.validTables;r++){if(!this.tables[r])continue;o.setAt(u++,r),c=this.tables[r].length,o.setAt(u++,c);for(h=0;h<c;h++)o.setAt(u++,TssCompiler._toUint8(this.tables[r][h]))}return s.buffer},TssCompiler.prototype._compile=function(){try{return this._parseLines(),this._parseDirectives(),this._parseChannels(),this._generateTsd()}catch(e){return Log.getLog().error(e.stack),Log.getLog().error(e.toString()),null}},TssCompiler.prototype.compile=function(e){return typeof e=="string"?this.source=TString.createFromString(e):this.source=TString.createFromUint8Array(new Uint8Array(e)),this._compile()};var MMLParser=function(){this.macro={},this.transpose=[0],this.endParse=!1,this.expandMacro=!0,this.noteShift=0,this.dump="",this.channel=0,this.asciiA="A".charCodeAt(0),this.commentMode=!1,this.FmNode=[],this.FmPipe=-1,this.FmOps=0,this.FmMode=!1,this.FmCount=0,this.number2name=["a","a+","b","c","c+","d","d+","e","f","f+","g","g+"],this.name2number={a:0,"a+":1,"b-":1,b:2,c:3,"c+":4,"d-":4,d:5,"d+":6,"e-":6,e:7,f:8,"f+":9,"g-":9,g:10,"g+":11,"a-":11}};MMLParser.prototype.isString=function(e){return typeof e=="string"||e instanceof String},MMLParser.prototype.repeat=function(e,t){var n="";for(var r=0;r<t;r++)n+=e;return n},MMLParser.prototype.getChannelString=function(){var e,t=this.channel%26,n=Math.floor(this.channel/26)-1;return n<0?e="#"+String.fromCharCode(this.asciiA+t):e="#"+String.fromCharCode(this.asciiA+n)+String.fromCharCode(this.asciiA+t),this.channel++,e},MMLParser.prototype.resetChannelString=function(){this.channel=0},MMLParser.prototype.expandWAVB=function(e){var t="";e.length%2==1&&(e+="0");for(var n=0;n<e.length;n+=2){n>0&&(t+=",");var r=e.substring(n,n+2),i=parseInt(r,16)-128;t+=i.toString(10)}return"<"+t+">"},MMLParser.prototype.parse=function(e){this.resetChannelString(),this.endParse=!1;var t=[];e=e.replace(/\n/g,"");var n=e.split(";");for(var r=0;r<n.length;r++){var i=n[r];this.noteShift=0;var s=this.parseLine(n[r],0);s.length>0&&t.push(s);if(this.endParse)break}return t},MMLParser.prototype.parseLine=function(e,t){var n=e;this.endParse=!1,t===undefined&&(t=0);var r=[];e.match(/#TITLE/)||(e=e.replace(/ /g,""));if(this.commentMode){if(!e.match(/^[^}]*}/))return[];e=e.replace(/^[^}]*}/,""),this.commentMode=!1}for(;;){if(this.expandMacro)while(e.match(/^([A-Z])(\([+\-]?[0-9]+\))?/)){var i=0;RegExp.$2!==""&&(i=parseInt(RegExp.$2.replace("(","").replace(")",""),10)),e=e.replace(/^([A-Z])(\([+\-]?[0-9]+\))?/,"");var s=this.parseLine(this.macro[RegExp.$1]+e,i);r=r.concat(s),e=""}else while(e.match(/^([A-Z])(\([+\-]?[0-9]+\))?/))e=e.replace(/^([A-Z])(\([+\-]?[0-9]+\))?/,""),r.push(["macro",RegExp.$1,RegExp.$2]);var o=e;if(e.match(/^(mp)([+\-]?[0-9]+)?(,[+\-]?[0-9]+)?(,[+\-]?[0-9]+)?(,[+\-]?[0-9]+)?(,[+\-]?[0-9]+)?/))e=e.replace(/^(mp)([+\-]?[0-9]+)?(,[+\-]?[0-9]+)?(,[+\-]?[0-9]+)?(,[+\-]?[0-9]+)?(,[+\-]?[0-9]+)?/,""),r.push(["mml",RegExp.$1,RegExp.$2,RegExp.$3,RegExp.$4,RegExp.$5,RegExp.$6]);else if(e.match(/^(na|nt)([+\-]?[0-9]+)?(,[+\-]?[0-9]+)?/))e=e.replace(/^(na|nt)([+\-]?[0-9]+)?(,[+\-]?[0-9]+)?/,""),r.push(["mml",RegExp.$1,RegExp.$2,RegExp.$3]);else if(e.match(/^(ns)([+\-]?[0-9]+)/))e=e.replace(/^(ns)([+\-]?[0-9]+)/,""),this.expandMacro||r.push(["exmml",RegExp.$1,RegExp.$2]),this.noteShift=parseInt(RegExp.$2,10);else if(e.match(/^(ml)([+\-]?[0-9]+)/))e=e.replace(/^(ml)([+\-]?[0-9]+)/,""),this.expandMacro||r.push(["exmml(unsupported)",RegExp.$1,RegExp.$2]);else if(e.match(/^([l])([+\-]?[0-9]+)/))e=e.replace(/^([l])([+\-]?[0-9]+)/,""),r.push(["mml",RegExp.$1,RegExp.$2]);else if(e.match(/^([sv])([+\-]?[0-9]+)?(,[+\-]?[0-9]+)?/))e=e.replace(/^([sv])([+\-]?[0-9]+)?(,[+\-]?[0-9]+)?/,""),r.push(["mml",RegExp.$1,RegExp.$2,RegExp.$3]);else if(e.match(/^([hijkmnopqrtuwxyz])([+\-]?[0-9]+)?/))e=e.replace(/^([hijkmnopqrstuvwxyz])([+\-]?[0-9]+)?/,""),r.push(["mml",RegExp.$1,RegExp.$2]);else if(e.match(/^([abcdefg][+\-]?)([0-9]+)?/)){e=e.replace(/^([abcdefg][+\-]?)([0-9]+)?/,"");var u=this.name2number[RegExp.$1];u=(u+t+this.noteShift+24)%12,r.push(["mml",this.number2name[u],RegExp.$2])}else if(e.match(/^([r])([0-9]*)/))e=e.replace(/^([r])([0-9]*)/,""),r.push(["mml",RegExp.$1,RegExp.$2]);else if(e.match(/^(\^)([0-9]*)/))e=e.replace(/^(\^)([0-9]*)/,""),r.push(["mml",RegExp.$1,RegExp.$2]);else if(e.match(/^(\/:)([0-9]*)/))e=e.replace(/^(\/:)([0-9]*)/,""),r.push(["mml",RegExp.$1,RegExp.$2]);else if(e.match(/^(:\/)/))e=e.replace(/^(:\/)/,""),r.push(["mml",RegExp.$1]);else if(e.match(/^(\[)([0-9]*)/))e=e.replace(/^(\[)([0-9]*)/,""),r.push(["mml",RegExp.$1,RegExp.$2]);else if(e.match(/^(\])/))e=e.replace(/^(\])/,""),r.push(["mml",RegExp.$1]);else if(e.match(/^([,.|$^()<>\/])/))e=e.replace(/^([,.|$^()<>\/])/,""),r.push(["mml",RegExp.$1]);else if(e.match(/^(@[a-z])([+\-]?[0-9]+)?/))e=e.replace(/^(@[a-z])([+\-]?[0-9]+)?/,""),r.push(["mml",RegExp.$1,RegExp.$2,RegExp.$3]);else if(e.match(/^(@[a-z])([+\-]?[0-9]+)?,([+\-]?[0-9]+)/))e=e.replace(/^(@[a-z])([+\-]?[0-9]+)?,([+\-]?[0-9]+)/,""),r.push(["mml",RegExp.$1,RegExp.$2,RegExp.$3]);else if(e.match(/^(%)([0-9]+)/))e=e.replace(/^(%)([0-9]+)/,""),r.push(["mml",RegExp.$1,RegExp.$2+" "]);else if(e.match(/^([@])([0-9]+)/))e=e.replace(/^([@])([0-9]+)/,""),r.push(["mml",RegExp.$1,RegExp.$2]);else if(e.match(/^{[^}]*}/))e=e.replace(/^{[^}]*}/,"");else if(e.match(/^{[^}]*$/))e=e.replace(/^{[^}]*$/,""),this.commentMode=!0;else if(e.match(/^(#OCTAVE|#VOLUME)(REVERSE|NORMAL)/))e=e.replace(/^(#OCTAVE|#VOLUME)(REVERSE|NORMAL)/,""),r.push(["directive",RegExp.$1,RegExp.$2]);else if(e.match(/^(#TABLE)([0-9]+),?(<[^>]*>)/))e=e.replace(/^(#TABLE)([0-9]+),?(<[^>]*>)/,""),r.push(["directive",RegExp.$1,RegExp.$2,RegExp.$3]);else if(e.match(/^(#WAV)([0-9]+),?(<[^>]*>)/))e=e.replace(/^(#WAV)([0-9]+),?(<[^>]*>)/,""),r.push(["directive",RegExp.$1,RegExp.$2,RegExp.$3]);else if(e.match(/^(#WAVB)([0-9]+),?([0-9a-fA-F]+)/))e=e.replace(/^(#WAVB)([0-9]+),?([0-9a-fA-F]+)/,""),r.push(["directive","#WAV",RegExp.$2,this.expandWAVB(RegExp.$3)]);else if(e.match(/^(#FINENESS)([0-9]+)/))e=e.replace(/^(#FINENESS)([0-9]+)/,""),r.push(["directive",RegExp.$1,RegExp.$2,RegExp.$3]);else if(e.match(/^(#TITLE) *(<[^>]*>)/))e=e.replace(/^(#TITLE) *(<[^>]*>)/,""),r.push(["directive",RegExp.$1,RegExp.$2]);else if(e.match(/^(#CHANNEL)([0-9]+)/))e=e.replace(/^(#CHANNEL)([0-9]+)/,"");else if(e.match(/^(#PRAGMA)(FAMICOM|GAMEBOY)/))e=e.replace(/^(#PRAGMA)(FAMICOM|GAMEBOY)/,""),r.push(["directive",RegExp.$1,RegExp.$2]);else if(e.match(/^(#MML)/))e=e.replace(/^(#MML)/,"");else{if(e.match(/^(#END)/)){e="",this.endParse=!0;break}if(e.match(/^(#FM)(.*)/))e=e.replace(/^(#FM)(.*)/,""),r.push(["directive",RegExp.$1,RegExp.$2]);else{if(!e.match(/^#([A-Z])=(.*)/)){e!==""&&(console.log("parse error:["+e+"]"),console.log("      >>> "+n),console.log("      >>> "+o));break}e=e.replace(/^#([A-Z])=(.*)/,""),this.expandMacro||r.push(["def",RegExp.$1,RegExp.$2]),this.macro[RegExp.$1]=RegExp.$2}}}return r},MMLParser.prototype.parseFmMacro=function(e){var t=0,n=[],r="",i,s;while(e.length>0)t===0?e.match(/^ +/)?e=e.replace(/^ +/,""):e.match(/^([A-Z])([0-8])?\(/)?(r="",i=RegExp.$1,RegExp.$2!==""?s=RegExp.$2:s="3",t++,e=e.replace(/^([A-Z])([0-8])?\(/,"")):e.match(/^([A-Z])/)?(n.push([RegExp.$1,"0",""]),e=e.replace(/^([A-Z])/,"")):e.match(/^\+/)&&(n.push("+"),e=e.replace(/^\+/,"")):e.match(/^\)/)?(t--,t===0?(n.push([i,s,this.parseFmMacro(r)]),e=e.substring(1)):(r+=e.charAt(0),e=e.substring(1))):e.match(/^\(/)?(t++,r+=e.charAt(0),e=e.substring(1)):(r+=e.charAt(0),e=e.substring(1));return n},MMLParser.prototype.evalFmMacro=function(e){this.FmNode=new Array(26),this.FmPipe=-1,this.FmOps=0;var t=this.parseFmMacro(e);this.evalFmMacroSub(t,0,1),this.FmMode=!0,this.FmCount=0},MMLParser.prototype.evalFmMacroSub=function(e,t,n){var r=1;for(var i=0;i<e.length;i++)if(this.isString(e[i]))if(e[i].match(/[A-Z]/)){this.FmOps++;var s=e[i].charCodeAt(0)-this.asciiA;this.FmNode[s]=[],node=this.FmNode[s],e[i+1]==="0"?(node[0]=0,node[1]=0,node[3]=this.FmPipe):(this.FmPipe++,node[0]=parseInt(e[i+1],10),node[1]=this.FmPipe,node[3]=this.FmPipe-1),t===1?(node[2]=0,node[3]=0):node[2]=n,i++}else e[i]==="+"&&(r=2);else this.evalFmMacroSub(e[i],t+1,r)},MMLParser.prototype.dumpFmMacro=function(e){var t=this.parseFmMacro(e);return this.dump="",this.dumpFmMacroSub(t,0),this.dump},MMLParser.prototype.dumpFmMacroSub=function(e,t){for(var n=0;n<e.length;n++)this.isString(e[n])?e[n].match(/[A-Z]/)&&(this.dump+=this.repeat(" ",t*4)+e[n]+"<--"+e[n+1]+"--\n",n++):this.dumpFmMacroSub(e[n],t+1)},MMLParser.prototype.compile=function(e,t){t===undefined&&(t=!0);var n=this.parse(e),r="<>",i="";for(var s=0;s<n.length;s++)if(n[s][0][0]==="directive"){var o=n[s][0][1];o==="#TITLE"?r=n[s][0][2]:o==="#TABLE"||o==="#WAV"?i+=n[s][0][1]+" "+n[s][0][2]+","+n[s][0][3]+"\n":o!=="#FM"&&(i+=n[s][0][1]+" "+n[s][0][2]+"\n")}for(var s=0;s<n.length;s++){if(n[s][0][0]==="directive"){var o=n[s][0][1];o==="#FM"&&this.evalFmMacro(n[s][0][2]);continue}t&&(i+=this.getChannelString()+"\n");var u=0;n[s][u][1]=="%"&&(i+=n[s][u][1]+n[s][u][2],u++);if(t&&this.FmMode){var a=this.FmNode[this.FmCount];i+="@i"+a[0]+","+a[1]+"@o"+a[2]+","+a[3],this.FmCount++,this.FmCount>=this.FmOps&&(this.FmMode=!1)}for(;u<n[s].length;u++)for(var f=1;f<n[s][u].length;f++)i+=n[s][u][f];i+="\n"}var l="";return t&&(l+="#TITLE "+r+"\n",l+="#CHANNEL "+Math.min(this.channel,26)+"\n"),l+=i,l},MMLParser.prototype.dump=function(e){var t=this.expandMacro;this.expandMacro=!1;var n=this.parse(e);this.expandMacro=t,console.log("=========================");for(var r=0;r<n.length;r++){for(var i=0;i<n[r].length;i++){var s="";for(var o=0;o<n[r][i].length;o++)s+=n[r][i][o]+" ";console.log(s)}console.log("-------")}console.log("=========================")},typeof module!="undefined"&&(module.exports=MMLParser),function(){var e=new MMLParser,t=new AudioLooper(2048),n=new MasterChannel;n.setVolume(1);var r=new TsdPlayer;r.device=new TssChannel,r.device.setPlayer(r),n.addChannel(r.device),r.setMasterChannel(n);var i="";document.onkeydown=function(s){var o=s.ctrlKey||s.metaKey;if(o&&s.keyCode===67){var u=String(document.getSelection());if(t.channel){t.setChannel(null);if(u===i)return}i=u;var a=new TssCompiler,f=e.compile(u);s.shiftKey&&console.log(f);var l=a.compile(f);l===null?console.log("MML compile error"):(t.setChannel(n),r.play(l))}}}();